...
print('Objective Metrics: Path length = {}, Cum. heading changes = {}'
      .format(pl, chc))


# plot their configuration
plt.figure(figsize=(8, 6))
plt.plot(agents[:, 0], agents[:, 1], ls='', marker='8', markersize=15,
         label='Sample agents')
...
ax.plot(0.0, 0.0, marker='8', markersize=20, color='r', linestyle='',
        label='Focal Agent')
ax.add_artist(Circle((0.0, 0.0), radius=3.6, color='r', fill=False, hatch='.',
              alpha=0.3, lw=2.))

for n in agents:
    heading = np.degrees(np.arctan2(n[3], n[2]))
    ax.add_artist(Ellipse((n[0] - focal_agent[0], n[1] - focal_agent[1]),
                  width=0.3, height=0.8, angle=heading, color='b',
                  fill=False, lw=1.5))
    ax.add_artist(Circle((n[0] - focal_agent[0], n[1] - focal_agent[1]),
                  radius=0.2, color='b'))
    ax.arrow(n[0] - focal_agent[0], n[1] - focal_agent[1], n[2], n[3], fc='b',
             ec='b', head_width=0.1, head_length=0.1)
